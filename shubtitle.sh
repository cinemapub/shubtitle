...
### Created by pforret ( pforret ) on 2023-11-28
### Based on https://github.com/pforret/bashew 1.20.4
...
option|W|width|subtitle width|1100
option|H|height|subtitle height|50
option|Y|vertical|Y position|975
option|F|font|font to use|Helvetica
option|S|start|start at N seconds|0
...
function Script:main() {
  ...
  case "${action,,}" in
    subtitle)
      ...
      #TIP: use «$script_prefix subtitle» to ...
      #TIP:> $script_prefix subtitle
      [[ ! -f "$input" ]] && IO:die "Input file [$input] not found"
      [[ -z "$output" ]] && IO:die "Need output file"

      # first create subtitle image
      sub_file="$tmp_dir/subtitle.png"
      convert -size "${width}x${height}" -background black -fill white -gravity Center -font "$font" -pointsize "$points" label:"$title" "$sub_file"
      # https://superuser.com/questions/713498/how-to-show-overlay-image-in-a-certain-time-span-with-ffmpeg
      ffmpeg -i "$input" -i "$sub_file" -filter_complex "[0:v][1:v] overlay=(W-w)/2:$vertical:enable='between(t,$start,$end)'" -y "$output" &> $log_dir/ffmpeg.log
      ;;
